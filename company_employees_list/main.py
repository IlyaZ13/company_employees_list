...
class Main(tk.Frame):                 #Создаем класс главного окна
    ...
    def init_main(self):      #Создаем функцию хранения и инициализации объектов графического интерфейса(панель инструментов)
        ...
        self.tree = ttk.Treeview(     
            self, columns=("ID", "name", "tel", "email", "salary"), height=45, show="headings"      #Создаем таблицу 
        )
    ...
    def records(self, name, tel, email, salary):        #Метод вызывающий метод записи
    ...
    def update_records(self, name, tel, email, salary):      #Метод для редактирования записей
...
class Child(tk.Toplevel):                                            #Класс отвечающий за дочерние окна
    ...
    def init_child(self):                                            #Метод инициализации и хранения объектов графического интерфейса(окно добавления сотрудника)
...
class Update(Child):        #Класс отвечающий за изменение данных уже существующих контактов сотрудников
    def __init__(self):     #Конструктор __init__(отвечает за инициализацию)
        super().__init__()
        self.init_edit()
        self.view = app
    ...
    def init_edit(self):      #Метод инициализации и хранения объектов графического интерфейса(окно редактирования контакта сотрудника)
        self.title("Редактирование контакта сотрудника")
...
class Search(tk.Toplevel):       #Класс отвечающий за поиск уже существующих данных
    ...
    def init_search(self):       #Метод инициализации и хранения объектов графического интерфейса(окно поиска cотрудника)
        self.title("Поиск контакта сотрудника")
...
class DB:            #Создаем класс базы данных
    ...
    def insert_data(self, name, tel, email, salary):       #Метод для добавления данных в базу данных
        self.cursor.execute(
